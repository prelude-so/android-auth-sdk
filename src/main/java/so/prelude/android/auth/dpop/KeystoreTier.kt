...
private const val LOG_TAG = "PreludeAuth"

/** AndroidKeystore alias for [domain]'s DPoP keypair. */
internal fun aliasFor(domain: String): String = "so.prelude.auth.dpop.$domain"
...
internal sealed interface KeystoreTier {
    ...
    data object StrongBox : KeystoreTier {
        override fun buildSpec(alias: String) = baseSpecBuilder(alias).setIsStrongBoxBacked(true).build()
    }
    ...
    companion object {
         ...
         */
        fun detect(): KeystoreTier =
            cached ?: synchronized(probeLock) {
        ...
        private fun probeTee(): Boolean =
            withProbeKey { alias ->
                val key = generateProbeKey(Tee.buildSpec(alias)) ?: return@withProbeKey false
                isHardwareBacked(key)
            }

        private fun generateProbeKey(spec: KeyGenParameterSpec): PrivateKey? =
            try {
                KeyPairGenerator
        ...
        private fun isHardwareBacked(privateKey: PrivateKey): Boolean =
            try {
        ...
        private inline fun withProbeKey(block: (alias: String) -> Boolean): Boolean {
            val alias = "so.prelude.auth.dpop.probe.${UUID.randomUUID()}"
            return try {
                block(alias)
            } finally {
                runCatching {
