...
/*
 * Step-up surface for [PreludeAuthClient].
 *
 ...
 * values); a violation surfaces as [PreludeAuthError.BadRequest].
 *
 * Throws [PreludeAuthError.MissingChallengeToken] when the server
 * returns a `continue` / `review` status without the promised
 * token, [PreludeAuthError.InvalidChallengeToken] when the token is
 * malformed or carries an unrecognised status, and
 * [PreludeAuthError.Forbidden] for `scope_not_allowed`. A
 ...
 */
suspend fun PreludeAuthClient.requestStepUp(
    scope: String,
    ...
    val request =
        buildSessionRequest("stepup/request")
    ...
    val status =
        PreludeStepUpStatus.fromWire(body.status)
 ...
 *
 * Throws [PreludeAuthError.InvalidChallengeToken] if [challenge]
 * is blocked (carries no token).
 */
suspend fun PreludeAuthClient.sendStepUpOTP(challenge: PreludeStepUpChallenge) {
 ...
 * [requestStepUp].
 *
 * Throws [PreludeAuthError.InvalidOTPCode] for a wrong / expired
 * code, [PreludeAuthError.InvalidChallengeToken] when [challenge]
 ...
 * [PreludeAuthError.MissingChallengeToken] if `/otp/check` returns
 ...
 * [PreludeAuthError.Unauthorized] — the post-completion refresh
 * detects the wiped stores and bails.
 */
suspend fun PreludeAuthClient.submitStepUpOTP(
 ...
 * [PreludeAuthError.Unauthorized] via [doRefresh]'s epoch guard.
 */
 ...
 *
 * Throws [PreludeAuthError.InvalidChallengeToken] for a missing /
...
private fun PreludeAuthClient.decodeChallenge(
    ...
    val challengeId =
        payload.stringField("challenge_id")
            ?: throw PreludeAuthError.InvalidChallengeToken(
...
private fun JsonObject.stringField(name: String): String? = (get(name) as? JsonPrimitive)?.takeIf { it.isString }?.contentOrNull
