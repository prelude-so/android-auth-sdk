...
/*
 * Change-password surface for [PreludeAuthClient].
 *
 ...
 * surface as [PreludeAuthError.InsufficientScope] and the local
 ...
 *      A vanilla `refresh()` racing in [PreludeAuthClient.inflightRefresh]
 ...
 * as [PreludeAuthError.InsufficientScope]. The scope is consumed by
 ...
 *   - [PreludeAuthError.InsufficientScope] when the session lacks
 *     `prld:pwd:write`.
 *   - [PreludeAuthError.InvalidPassword] when the new password fails
 *     the server's policy.
 *   - [PreludeAuthError.BadRequest] when password auth is not
 *     configured / disabled for the app.
 *   - [PreludeAuthError.Forbidden] for `auth_blocked` and other
 *     policy denials.
 *   - [PreludeAuthError.Unauthorized] when the session is expired
 *     and the auto-refresh interceptor cannot recover it.
 *   - [PreludeAuthError.RateLimited] when the per-session bucket is
 *     exhausted.
 *   - [PreludeAuthError.Network] / [PreludeAuthError.Timeout] for
 *     transport failures.
 *   - [PreludeAuthError.InternalServerError] for 5xx.
 ...
 * refresh in [PreludeAuthClient.inflightRefresh] so the next mint
 ...
 * handling as [PreludeAuthClient.dispatchSignalsIfConfigured].
 *
 ...
 */
private suspend fun PreludeAuthClient.dropConsumedScopeAfterChangePassword() {
    try {
