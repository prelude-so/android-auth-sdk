...
import so.prelude.android.auth.crypto.fromJwt
...
import so.prelude.android.auth.http.NowProvider
import so.prelude.android.auth.http.RefreshTokenResponse
import so.prelude.android.auth.http.StepUpRefreshRequestBody
...
/**
 * Client for the Prelude Auth API.
 *
 ...
 */
class PreludeAuthClient internal constructor(
     ...
     * surface a spurious [PreludeAuthError.Unauthorized]. Same slot-
     ...
     * as [PreludeAuthError.InvalidConfiguration] without first
     ...
     *
     * Throws the JWT decoder's [PreludeAuthError.InvalidChallengeToken]
     * if the token is malformed; the refresh path goes through
     * [makeUserForRefresh] to surface this as
     * [PreludeAuthError.RefreshFailed] per the documented contract.
     */
     ...
     * surface as [PreludeAuthError.RefreshFailed] rather than the
     * JWT decoder's internal [PreludeAuthError.InvalidChallengeToken].
    ...
    private fun makeUserForRefresh(accessToken: String): PreludeUser =
        try {
            ...
            throw PreludeAuthError.RefreshFailed(
    ...
    private companion object {
         ...
         * [PreludeAuthError.InvalidConfiguration] when neither
