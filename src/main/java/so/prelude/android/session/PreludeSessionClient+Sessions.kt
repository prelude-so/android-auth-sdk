...
import so.prelude.android.session.http.ListSessionsResponse
import so.prelude.android.session.http.SessionViewResponse
import java.time.Instant
import java.time.format.DateTimeParseException

/*
 * List + revoke surfaces for [PreludeSessionClient].
 *
 ...
 * [PreludeSessionError.Unauthorized] instead of resurrecting the
 ...
 *
 * Concurrent callers serialise on [PreludeSessionClient.revokeMutex]
 ...
 * ([PreludeSessionError.Network], [PreludeSessionError.Timeout]),
 * [PreludeSessionError.Unauthorized] when the session is expired and
 * the auto-refresh interceptor cannot recover it, and
 * [PreludeSessionError.Generic] (`code = "decoding_failed"`) when a
 ...
 */
suspend fun PreludeSessionClient.listSessions(
    ...
    val (body, _) = httpClient.sendJson(
        request = request,
 ...
 * [PreludeSessionClient.sessionEpoch] — so a refresh racing the wipe
 * surfaces as [PreludeSessionError.Unauthorized] instead of
 ...
 * ([PreludeSessionError.Network], [PreludeSessionError.Timeout],
 * [PreludeSessionError.Unauthorized], [PreludeSessionError.RateLimited],
 * [PreludeSessionError.Forbidden]) and
 ...
 */
suspend fun PreludeSessionClient.revokeSessions(target: PreludeRevokeTarget) =
    revokeMutex.withLock {
        ...
        if (revocationTouchesCurrentSession(target, priorSessionId)) {
            ...
            // see the file header in `PreludeSessionClient+Logout.kt`.
 ...
 * structured [PreludeSessionError.Generic] in [listSessions] — the
 ...
 * UI symptom.
 */
private fun SessionViewResponse.toPublic(): PreludeSessionView = PreludeSessionView(
    id = id,
