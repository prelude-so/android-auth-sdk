...
internal class RefreshTokenStore(
     ...
     * [SessionTokenStoreError.CodecFailure] if the record can't be
     * encoded (impossible for the current schema — guard against a
     * future field with a non-default serializer) and
     * [SessionTokenStoreError.StorageFailure] when the backend
     * refuses the write.
     */
    fun set(domain: String, record: RefreshTokenRecord) {
        val blob = try {
     ...
     * Throws [SessionTokenStoreError.StorageFailure] on any other
     * backend failure.
     */
