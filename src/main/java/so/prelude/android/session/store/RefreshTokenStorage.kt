...
private const val PREFS_FILE = "so.prelude.session.refresh"
 ...
 * [SharedPreferences].
 *
 * Stored in a dedicated prefs file (`so.prelude.session.refresh`)
 ...
 * rejection propagates as [SessionTokenStoreError.StorageFailure]
 ...
 */
internal class SharedPreferencesRefreshTokenStorage(context: Context) : RefreshTokenStorage {
    ...
    override fun write(domain: String, blob: String) {
        val ok = try {
            prefs.edit().putString(domain, blob).commit()
        } catch (e: Exception) {
            throw SessionTokenStoreError.StorageFailure(e)
        }
        if (!ok) {
            throw SessionTokenStoreError.StorageFailure(
