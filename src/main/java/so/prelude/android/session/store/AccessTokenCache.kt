...
internal class AccessTokenCache(
     ...
     */
    fun hydrate(domain: String) = lock.withLock {
     ...
     * boundary.
     */
    fun get(domain: String): AccessTokenEntry? = lock.withLock {
     ...
     * flight.
     */
    fun getWithoutExpirationCheck(domain: String): AccessTokenEntry? = lock.withLock {
        memory[domain]
    }
     ...
     */
    fun set(domain: String, entry: AccessTokenEntry) = lock.withLock {
     ...
     * cold start.
     */
    fun invalidate(domain: String) = lock.withLock {
     ...
     */
    fun clear(domain: String) = lock.withLock {
    ...
    private fun writePersisted(domain: String, entry: AccessTokenEntry) {
        val blob = try {
            cacheJson.encodeToString(AccessTokenEntry.serializer(), entry)
        } catch (e: SerializationException) {
            throw SessionTokenStoreError.CodecFailure("encode failed", e)
        }
