 ...
 * cross-instance races (two `PreludeSessionClient`s in the same
...
internal class AndroidKeystoreStore(
    ...
    override fun getOrCreate(domain: String): DPoPKey = domainLock(domain).withLock {
     ...
     * keeps that intact.
     */
    override fun delete(domain: String) = domainLock(domain).withLock {
        try {
    ...
    override fun setNonce(domain: String, nonce: String) = nonceStore.set(domain, nonce)
    ...
    private fun findKey(domain: String): DPoPKey? {
        ...
        // the alias.
        val entry = raw as? KeyStore.PrivateKeyEntry
    ...
    companion object {
         ...
         */
        private fun domainLock(domain: String): ReentrantLock =
            locks.computeIfAbsent(domain) { ReentrantLock() }
    }
}
