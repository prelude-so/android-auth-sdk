...
import so.prelude.android.session.http.PasswordCompliancyResponse

/*
 * Password-compliancy surface for [PreludeSessionClient].
 *
 * Three entry points:
 *
 ...
 * [PreludeSessionClient.buildSessionRequest] handles this when the
 ...
 * [PreludeSessionError.BadRequest] when password auth is not configured
 * for the app, and [PreludeSessionError.Generic] for any other unmapped
 * server code.
 */
suspend fun PreludeSessionClient.getPasswordCompliancy(): PreludePasswordCompliancy {
 ...
 */
suspend fun PreludeSessionClient.validatePassword(
    password: String,
 ...
 * (`Character.getType`), so non-ASCII letters land in
 * [PreludePasswordCompliancyCriterion.UPPERCASE] /
 * [PreludePasswordCompliancyCriterion.LOWERCASE] (e.g. Greek `Α` is
 * uppercase, `β` is lowercase) rather than being miscounted as
 * symbols.
 *
