...
/*
 * Password login surface for [PreludeSessionClient].
 *
 ...
 * [PreludeSessionError.SignalsDispatchFailed] (wrapping the underlying
 * cause) — silently shipping a login without anti-fraud coverage would
 * be the worst possible failure mode.
 *
 * Distinguishes [PreludeSessionError.InvalidPassword] (the password
 * failed the server's policy — fix is a stronger password) from
 * [PreludeSessionError.Unauthorized] (bad credentials — fix is a
 * different password) so callers can branch on the recovery path,
 * and surfaces [PreludeSessionError.MissingChallengeToken] when
 ...
 * [PreludeSessionError.BadRequest] for an invalid email shape,
 * [PreludeSessionError.RateLimited] when the login bucket is exhausted,
 * [PreludeSessionError.Forbidden] for `auth_blocked`, and
 * [PreludeSessionError.InvalidChallengeToken] from the `/login/finalize`
 * hop. Network failures surface as [PreludeSessionError.Network] /
 * [PreludeSessionError.Timeout]; unmapped server codes as
 * [PreludeSessionError.Generic].
 *
 ...
 * [PreludeSessionClient.sessionEpoch] guard inside [finalizeLogin]:
 * a concurrent [logout] that bumps the counter mid-call surfaces as
 * [PreludeSessionError.Unauthorized] instead of resurrecting the session
 * the caller just revoked.
 */
suspend fun PreludeSessionClient.loginWithPassword(
    ...
    // boundary.
    val request = buildSessionRequest("login/email/password")
