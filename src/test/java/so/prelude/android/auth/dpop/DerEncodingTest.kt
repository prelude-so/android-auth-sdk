...
class DerEncodingTest {
     ...
     * `PreludeAuthError.Timeout` was fixed for).
     */
    @Test
    fun malformedSignature_isFreshInstancePerThrow() {
        ...
        val first =
            runCatching { derToRawEcdsa(firstBad, coordinateBytes = 32) }
                .exceptionOrNull()
        val second =
            runCatching { derToRawEcdsa(secondBad, coordinateBytes = 32) }
                .exceptionOrNull()
