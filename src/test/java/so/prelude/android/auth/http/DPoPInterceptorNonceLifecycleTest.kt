...
class DPoPInterceptorNonceLifecycleTest {
     ...
     */
    @Test
    fun harvestedNonce_isReusedOnSubsequentCall() =
        runTest {
    ...
    @Test
    fun proof_omitsNonceOnFirstRequestForFreshDomain() =
        runTest {
            val store = FakeDPoPKeyStore()
            val interceptor = DPoPInterceptor(store, domain)
     ...
     */
    @Test
    fun successful2xx_freshNonceHeader_overridesCachedNonce() =
        runTest {
     ...
     * [so.prelude.android.auth.dpop.DPoPNonceStore]). On the
     ...
     */
    @Test
    fun coldStart_reusesPersistedNonce_withoutChallengeRoundTrip() =
        runTest {
    ...
    @Test
    fun emptyStoredNonce_isTreatedAsAbsent() =
        runTest {
