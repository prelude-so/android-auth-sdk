...
internal class FakeDPoPKey(
    private val jwk: Map<String, String> = mapOf(
        "kty" to "EC",
        "crv" to "P-256",
...
internal class FakeDPoPKeyStore : DPoPKeyStore {
    ...
    override fun getOrCreate(domain: String): DPoPKey =
        keys.computeIfAbsent(domain) { keyFactory() }
    ...
    override fun setNonce(domain: String, nonce: String) {
        nonces[domain] = nonce
    }
    ...
    fun setKey(domain: String, key: DPoPKey) {
        keys[domain] = key
    }
}
