...
import so.prelude.android.session.http.HttpSession
...
internal class StubHttpSession : HttpSession {
    ...
    fun install(path: String, canned: Canned) = lock.withLock {
        byPath[path] = canned
    }

    /** Convenience: install the same response for several paths. */
    fun installAll(vararg pairs: Pair<String, Canned>) = lock.withLock {
     ...
     */
    fun installGate(path: String) = lock.withLock {
