...
class ChallengeDPoPInterceptorTest {
    ...
    private fun mkRequest(): Request =
        Request.Builder().url("https://api.example.com/v1/step-up/finalize").build()

    private fun mkResponse(request: Request, code: Int = 200, nonce: String? = null): Response {
    ...
    @Test
    fun noKeyForDomain_passesThroughUnchanged() = runTest {
    ...
    @Test
    fun missingJtiInToken_passesThroughUnchanged() = runTest {
        val store = FakeDPoPKeyStore()
    ...
    @Test
    fun keyAndJtiPresent_signsProofWithChallengeJti() = runTest {
        val store = FakeDPoPKeyStore()
        store.setKey(domain, FakeDPoPKey())
    ...
    @Test
    fun nonceFromResponse_isNotHarvested() = runTest {
        val store = FakeDPoPKeyStore()
        store.setKey(domain, FakeDPoPKey())
        val interceptor = ChallengeDPoPInterceptor(store, domain, challengeToken("j"))
     ...
     */
    @Test
    fun hostOverride_withPort_doesNotThrow() = runTest {
        val store = FakeDPoPKeyStore()
        store.setKey(domain, FakeDPoPKey())
        val interceptor = ChallengeDPoPInterceptor(
            keyStore = store,
            domain = domain,
